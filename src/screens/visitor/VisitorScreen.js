...
import { Text, View, BackHandler, ToastAndroid, StyleSheet, Image, TouchableOpacity, FlatList, TextInput, TouchableWithoutFeedback, Keyboard } from 'react-native';
...
const VisitorScreen = ({ navigation }) => {
    ...
    console.log(userData) // see this? console log it without stringify
    ...
    const renderItem = ({ item }) => {
        return (
            ...
            >
                {item.img_url && <Image source={{ uri: item.img_url }} style={styles.image} />}
                <View style={styles.cardContent}>
                    <Text style={styles.cardTitle}>{item.model}</Text>
                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                        <PesoSign name='peso-sign' size={hp('1.8%')} color='#85929E' />
                        <Text style={styles.cardDescription}>{`${Intl.NumberFormat('PH').format(item.price)}`}</Text>
                    </View>
                </View>
            </TouchableOpacity>
        );
            ...
            <View style={styles.container}>
                <View style={styles.header}>
                    <View style={styles.profile}>
                        <TouchableOpacity style={[styles.profileIcon, { backgroundColor: userData.profile_color }]} onPress={() => navigation.navigate('VisitorSettingScreen')}>
                            <Text style={styles.profileInitial}>{userData.first_name[0]}</Text>
                        </TouchableOpacity>
                        <Text style={styles.profileName}>{userData.first_name}</Text>
                    </View>
                    <View style={styles.notificationIcon}>
                        <TouchableOpacity onPress={() => navigation.navigate('VisitorNotificationScreen')}>
                            <MaterialIcon name='notifications' size={hp('3%')} color='#5D6D7E' />
                        </TouchableOpacity>
                    </View>
                </View>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginTop: hp('3%') }}>
                    ...
                    </View>
                    {
                        search.length >= 1 && (
                            <View>
                                <MaterialIcon name='cancel' size={20} color='#5D6D7E' onPress={() => setSearch('')} />
                            </View>
                        )
                    }
                </View>
                ...
                <View style={{ flexDirection: 'row', gap: 10 }}>
                    <View style={{ backgroundColor: '#f6f8fa', height: hp('14%'), width: wp('45%'), borderRadius: 6, justifyContent: 'center', alignItems: 'center' }}>
                        <Text style={{ color: '#5D6D7E', fontSize: hp('1.8%') }}>Amenities</Text>
                    </View>
                    <TouchableOpacity
                        ...
                        style={{ backgroundColor: '#f6f8fa', height: hp('14%'), width: wp('45%'), borderRadius: 6, justifyContent: 'center', alignItems: 'center' }}>
                        <Text style={{ color: '#5D6D7E', fontSize: hp('1.8%') }}>Homeowner</Text>
                    </TouchableOpacity>
                </View>
